...
markup_retry = telebot.types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
...
button_retry = telebot.types.KeyboardButton('Выполнить')
markup_retry.add(button_retry)

rapidapi = RapidApi(api_key=rapidapi_key)

help_text = 'Нажмите на одну из кнопок или используй одну из следующих команд:\n\n' \
            ...
            '/history или кнопка "История поиска" — вывод истории поиска отелей (последние 10 запросов)\n' \
